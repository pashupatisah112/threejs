<template>
  <div>
    <p>Different loaders in three js for different kinds of model</p>
    <small>Recommended types are .GLB and .GLTF</small>
    <div ref="canvas"></div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      sphereMaterial: new THREE.MeshBasicMaterial({
        color: 0x000229,
        wireframe: true, //wireframe true to view skeleton
      }),
      box: null,
    };
  },
  async mounted() {
    ...
    // const toy=new URL('../assets/models/godzilla.gltf',import.meta.url)
    //    const toy=await new URL('godzilla.gltf',import.meta.url)

    // console.log(toy)
    const assetLoader = await new modelLoader.GLTFLoader();
    assetLoader.load('model/wooden_table/scene.gltf',
      (gltf) => {
